...
export default class HUD extends Phaser.GameObjects.GameObject{
  ...
  constructor(scene : Phaser.Scene, colorControl : ColorControl){
    ...
    this.nextBallUI = this.scene.add.image(GameScreen.getInstance().getRelativePosX(scene.cameras.main.centerX + 300), GameScreen.getInstance().getRelativePosY(scene.cameras.main.height - this.height/2), "bubble");
    this.nextBallUI.setScale(0.5);
    this.nextBallUI.setTint(this.colorControl.getNextColor());
    this.scoreUI = this.scene.add.text(GameScreen.getInstance().getRelativePosX(20), GameScreen.getInstance().getRelativePosY(Constant.DEFAULT_HEIGHT-100), "Score : 0", {fontSize:"36px", fontFamily:"Arial", color:"#000000"});
    this.nextBallText = this.scene.add.text(GameScreen.getInstance().getRelativePosX(456), GameScreen.getInstance().getRelativePosY(Constant.DEFAULT_HEIGHT-100), "Next Ball", {fontSize:"36px", fontFamily:"Arial", color:"#000000"});
    ...
    this.gameOverLimit = new DashedLine(scene, GameScreen.getInstance().getRelativePosX(0), GameScreen.getInstance().getRelativePosY(scene.cameras.main.height-380), GameScreen.getInstance().resizeX(Constant.DEFAULT_WIDTH), GameScreen.getInstance().resizeY(Constant.DEFAULT_HEIGHT-380), 0xffffff, GameScreen.getInstance().resize(10), GameScreen.getInstance().resize(10), GameScreen.getInstance().resize(10));
  }
